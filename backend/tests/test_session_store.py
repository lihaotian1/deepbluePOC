...
def test_update_chunks_clears_saved_compare_results() -> None:
    ...
    saved = store.save_results(session.doc_id, [build_result(chunk_id=1, content="original")])

    assert saved is not None
    assert len(saved.compare_results) == 1
    ...
    assert updated.compare_results == []
    assert store.get(session.doc_id) is not None
    assert store.get(session.doc_id).compare_results == []
